...
public class Window_Graph : MonoBehaviour {
    ...
    [SerializeField] private Sprite circleSprite1;
    ...
    private void Awake() {
        ...
        List<int> valueList = new List<int>() { 40, 44, 48, 52, 56, 52,48, 44, 40, 44, 48, 52, 56, 52, 48, 44, 44};
        ShowGraph(valueList, (int _i) => (_i + 1) + "AM", (float _f) => Mathf.RoundToInt(_f) + "-" + Mathf.RoundToInt(_f + 50));

        List<int> valueList2 = new List<int>() { 49, 64, 42, 57, 58, 52, 68, 34, 50, 46, 42, 52, 56, 52, 48, 44, 44 };
    ...
    private GameObject CreateCircle(Vector2 anchoredPosition) {
       ...
       gameObject.GetComponent<Image>().sprite = circleSprite1;
    ...
    private void ShowGraph2(List<int> valueList2, Func<int, string> getAxisLabelX = null, Func<float, string> getAxisLabelY = null)
    {
        if (getAxisLabelX == null)
        {
            getAxisLabelX = delegate (int _i) { return _i.ToString(); };
        ...
        float graphHeight = graphContainer.sizeDelta.y;
        float yMaximum = 100f;
        float xSize = 50f;   // initialy it is 50, I made it 5

        GameObject lastCircleGameObject = null;
        for (int i = 0; i < valueList2.Count; i++)
        {
            float xPosition = xSize + i * xSize;
            float yPosition = (valueList2[i] / yMaximum) * graphHeight;
            GameObject circleGameObject = CreateCircle(new Vector2(xPosition, yPosition));
            if (lastCircleGameObject != null)
            {
    ...
    private void CreateDotConnection2(Vector2 dotPositionA, Vector2 dotPositionB)
    {
        GameObject gameObject = new GameObject("dotConnection", typeof(Image));
        gameObject.transform.SetParent(graphContainer, false);
        gameObject.GetComponent<Image>().color = Color.blue; /*   new Color(1,1,1, .5f);  */
        RectTransform rectTransform = gameObject.GetComponent<RectTransform>();
        Vector2 dir = (dotPositionB - dotPositionA).normalized;
        float distance = Vector2.Distance(dotPositionA, dotPositionB);
        rectTransform.anchorMin = new Vector2(0, 0);
